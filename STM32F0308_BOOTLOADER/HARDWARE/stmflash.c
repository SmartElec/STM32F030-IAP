...
{ 			 		 
	...
	for(i=0;i<NumToWrite;i++)
	{
		...
		if((ret==FLASH_ERROR_WRP)||(ret==FLASH_ERROR_PROGRAM))
		{
			return 1;
		}
		//printf("FLASH_Status=%d\r\n",ret);
	    WriteAddr+=2;//地址增加2.
	}  
	return 0;
} 
...
u16 STMFLASH_Write(u32 WriteAddr,u16 *pBuffer,u16 NumToWrite)	
{
	...
	while(1) 
	{
		...
		if(i<secremain)//需要擦除
		{
			...
			if(STMFLASH_Write_NoCheck(secpos*STM_SECTOR_SIZE+STM32_FLASH_BASE,STMFLASH_BUF,STM_SECTOR_SIZE/2))//写入整个扇区  
			{
				FLASH_Lock();//上锁
				return 1;
			}
		...
		{
			if(STMFLASH_Write_NoCheck(WriteAddr,pBuffer,secremain))//写已经擦除了的,直接写入扇区剩余区间.
			{
				FLASH_Lock();//上锁
				return 1;
			}				
...
{
	u16  OptionByte=OptionByte_Read();
	if(OptionByte==data)
		return;
	FLASH_Unlock();//解锁
	FLASH_OB_Unlock();
...
void OptionByte_Write_A5A5(void)
{
	OptionByte_Write(0xa5a5);
}
